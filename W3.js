...
//        switch (pet) {
//         case "dog":
//              console.log(`Me pet ${pet} goes WOOF WOOF!`)
//         break
//         case "cat":
//              console.log(`Me pet ${pet} goes MEOW MEOW!`)
//         break
//         case "bird":
//             console.log(`Me pet ${pet} goes TWEET TWEET!`)
//         break
//         case "snake":
//             console.log(`Me pet ${pet} goes HISS HISS!`)
//         break
//         default:
...
// undefined + 6 // NaN -> JavaScript  can not convert undefined into a number type, resulting in the result being not a number.
// null + 5 // 5 -> JavaScript can convert null to a number type with equates to 0, resulting in 0 + 5
// "Hello" + null // “Hellonull” -> JavaScript converts null into a string type and add both strings together.
...
// String(true) // "true" -> JavaScript converts the true from an boolean type to a string type of "true".
...
// //BigIn can represent numbers values outside these parameters.
// 900719925124740998n;
// //Boolean -> Logical data type which can either be true or false, often used to decide which block of code is to be executed within an conditional statement.
//     2 + 2 === 4 //true
//     3 + 3 === 5 //false
...
pop() // -> "Removes the last item of an array and return it" (Ayodeji 2019).
console.log(fruits.pop()) // -> lemons
shift() // -> "Removes the first item of an array and return it" (Ayodeji 2019).
...
class Car {
   ...
   //The below constructor creates the object "brand" as an instance of the car class. 
	constructor(brand) {
      ...
      // And so whenever the constructor is called, the newly created objected will be the returned value.
		this.carname = brand;
	}
...
class Model extends Car {
   ...
   // This allows Model to call the present() function and have access to the instance object of brand within the carname attribute.
	show() {
  	// When the show() function is called, it will return the the present function()
   // and concatenate it with a new string utilising  through the "this" keyword the instance object stored within model.
...
// The constructor is than to iterate through the array, and at every index denoted by x, i is than incremented by 1 starting at 1980.
// This results in the first index equalling to 1980, and index [40] to equalling to 2019.
// Once the constructor creates the array,
let models = Array.from(new Array(40), (x, i) => i + 1980);
// The function randomIntFromInterval is declared and takes two arguments.
// The firs argument is denoted by min and takes a minimal number.
...
function randomIntFromInterval(min, max) {
   ...
   // THe Math.random() method generates a random number between the values of min and max.
	// The Math.floor() method that returns the nearest integer value.
...
for (model of models) {
   ...
   // This denotes index [0] of the makes and models array.
   // The value of makes.length - 1 is used as the functions max argument.
   ...
   // Whenever make or model are called, the for of statement iterates through the respected arrays (makes for when make is called) and (models for when model is called).
   // The variable then calls the randomIntFromInterval function to generate a random element within the arrays, (car brand brand in makes) and (year in models).
   ...
   // All 40 possible years within the models array are iterated through and assigned one of the three makes array strings.
   ...
   // The show() function is able to be accessed by mycar because although it is a new instance of the class model and so it inherits its properties and functions.
   // And because the model class is a child of the Class Car, it has access to its functions.
